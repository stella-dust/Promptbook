...
- `npm run check`：176 条正式记录，0 错误、0 警告、1 个既有弃用提示；`npm test`：37/37；`npm run build`：182 个静态页面；`git diff --check` 通过。1280px 本地生产构建截图可见首页和 No.119 详情的实际图片；检索“野兽派”与建筑分类联动返回 1 条；390px 手机截图无横向溢出。部署、线上 build-info 与博客回归检查待本次推送后补记。
